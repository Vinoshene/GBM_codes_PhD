clear all;
%load('test');
[selection,path_in,filterI] = uigetfile ('*.*','Load a file','C:\Users\Asus\Desktop'); % C:\Users\Asus\Desktop
%shuttleVideo = VideoReader(selection);
pathAndFileIn = [path_in selection];
tic
numberofframes=1200;        %size(shuttleVideo,3);                     %=floor((duration)*(framerate));

for i=1:numberofframes
    tmp = imread(pathAndFileIn,'Index',i);
    tmp1 = binImage(tmp,1); %4
    %tmp=tmp-offsettif;
    Y(:,:,i)=tmp1;
end
